...
function tevkori_responsive_shortcode($atts) {
	...
	$style='';
	if($float=='none' && $textAlign=='left'):
		$style='width:100%;text-align:left';
	elseif($float=='none' && $textAlign=='right'):
		$style='width:100%;float:right;text-align:right';
	elseif($float=='none' && $textAlign=='center'):
		$style='width:100%;margin: auto;text-align:center;float:right;';	
	elseif($float=='left' && $textAlign=='right'):
		$style='float:left;text-align:left';
	elseif($float=='left' && $textAlign=='left'):
		$style='float:left;text-align:left';		
	elseif($float=='right' && $textAlign=='right'):
		$style='float:right;text-align:right';	
	elseif($float=='' && $textAlign=='left'): 
		$style='width:100%;text-align:left';
	else:
	$style='width:100%;text-align:left';
	endif;
   ...
   return
		'<picture style="'.$style.'" data-alt="'. tevkori_get_img_alt($imageid ) .'">
...
function tevkori_responsive_insert_image( $html, $id, $caption, $title, $align, $url ) {
	return "[responsive imageid='$id' size1='63' size2='222' size3='250' size4='515' size5='575' float='none' align='left']";
}
