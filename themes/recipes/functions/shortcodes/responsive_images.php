...
function get_custom_picture_srcs( $image, $mappings ) {
	...
	foreach ( $mappings as $key => $val ) { 
		$size=explode('=>', $val);
			...
			$arr[] = '<source data-pin-media="'.$default[0].'" srcset="'. $image_src[0] . ' "media="(min-width:'. $size[0].'px)" />';
	}
	return implode($arr );
}
...
function responsive_shortcode_function($atts) {
   ...
   return
		'<picture title="'. tevkori_get_img_alt($imageid ) .'" alt="'. tevkori_get_img_alt($imageid ) .'" data-alt="'. tevkori_get_img_alt($imageid ) .'">
